...
// Run: node experiments/short-arcs/fetch-horizons.mjs [--offline] [--out DIR]
...
function options(argv) {
  const result = {offline: false, out: fileURLToPath(new URL('./fixtures/', import.meta.url))};
...
async function main() {
  ...
  const cases = [];
  for (const body of protocol.bodies) {
    for (const date of protocol.datesUtc) {
      const id = body.name.toLowerCase() + '-' + date.slice(0, 10);
      const start = jdUtc(date);
      const obsTimes = protocol.observationMinutesAfterStart.map(minutes => start + minutes / 1440);
      const lastTime = obsTimes.at(-1);
      const heldoutTimes = protocol.holdoutHoursAfterLastObservation.map(hours => lastTime + hours / 24);
      ...
      const observer = await response(out, id, 'observer', {...common, CENTER: protocol.observer,
      ...
      cases.push({id, split: body.split, object: {number: body.number, name: body.name, class: body.class},
        observer: protocol.observer, startJdUtc: start, startJdTdb, startJdTt, epochJdUtc, epochJdTdb, epochJdTt,
        ...
        rawFiles: [observer.record, elementsResponse.record, stateResponse.record]});
  ...
  const rawRecords = cases.flatMap(item => item.rawFiles);
  const corpus = {schemaVersion: 1, provenance: {
    ...
    protocolFile: 'protocol.json', protocolSha256: hash(protocolText),
  ...
  }, splitPolicy: protocol.splitPolicy, protocol, cases};
  await writeFile(resolve(out, 'corpus.json'), json(corpus));
