...
export function commandFor(c,arc,kind,settings={noise:'exact',seed:0},budget=100) {
  ...
  // Pinned ephem0.cpp applies this switch only while producing ordinary
  // ephemerides, then restores internal differential solar light deflection.
  // Horizons quantity 1 omits that term; fitting raw Horizons inputs still has
  // a convention mismatch because the internal observation model retains it.
  command.files['/job/job.env']+='DISABLE_LIGHT_BENDING=1\n';
  if(kind==='fit') {
  ...
  return {...timeGrid(command,truth),kind,designation:'REG0001',epochTt:epoch.jdTt,observationCount:arc.observationIndices.length,
    ...
    stateFrame:'heliocentric geometric IAU76/J2000 ecliptic AU and AU/day at TT',
    skyFrame:'ICRF geocentric forecast degrees with light-time correction and differential solar light deflection disabled, UTC reception; Horizons quantity 1 convention',
    lightDeflection:{reference:'Horizons quantity 1 omits differential solar light deflection.',
      forecast:'DISABLE_LIGHT_BENDING=1 disables differential solar light deflection for ephemerides only.',
      fitting:kind==='fit'?'Internal differential solar light deflection remains enabled. Raw Horizons inputs therefore retain a convention mismatch; fit truth errors are report-only and these inputs are not pure synthetic CCD astrometry.':null},
